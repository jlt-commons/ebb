...
;; missionary as test/lolcat/lib.cljc. EPL 2.0.
;;
;; Ported to jolt: reader conditionals stripped -- `catch :default` ->
;; `catch Throwable`, and the cljs `-invoke`/`-deref` protocol names -> the
;; `clojure.lang.IFn`/`IDeref` shapes jolt's deftype takes. Otherwise verbatim.
(ns lolcat.lib
  (:require
   [clojure.string :as s]
   [clojure.test :as t]
