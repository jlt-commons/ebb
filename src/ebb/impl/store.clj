;; Derived from missionary/impl/Store.cljs (missionary, EPL 2.0).
;; See Store.java for the synchronisation strategy (ADR-001 rule 1).
(ns ^:no-doc ebb.impl.store
...
(deftype Port [sg ^:unsynchronized-mutable state]
  clojure.lang.IFn
  (invoke [this]
    (freeze this))
  (invoke [this x]
...
(deftype Reader [port step done ^:unsynchronized-mutable state ^:unsynchronized-mutable last]
  clojure.lang.IFn
  (invoke [this]
    (cancel this))
  clojure.lang.IDeref
...
(defn concurrent []
  (throw (ex-info "Concurrent store reader." {})))
