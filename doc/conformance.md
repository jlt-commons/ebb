...
- **An uncaught error in an `ap` branch does not fail the whole `ap`** (`ebb-8nq.38`). `Ambiguous.java`'s `transfer` nils the notifier, kills the process and drops the queue when the branch it is delivering produced a `Throwable`, so branches still in flight are discarded. Ebb delivers the error and carries on. Verified against missionary itself: `(m/ap (m/amb 1 (throw …) 3))` drained by a consumer that does not cancel gives `[:val 1] :done [:err "boom"]` there and `[:val 1] [:err "boom"] :done [:val 3]` here.
- **`latest` over a discrete input hangs the runtime** (`ebb-8nq.37`). `m/latest` needs continuous inputs and should report `Uninitialized continuous flow` for one that has no value, as `m/sample` reports `Undefined continuous flow`. Given an `ap` it never completes, and it takes the process with it — a main thread waiting on a 3s promise deref elsewhere never gets its answer either, so this is a pinned carrier rather than one stuck process. `run` cancels every input when it finds one uninitialised, and that cancel path is the suspect.
